...
class StringObfuscateProcessor(
    ...
    private fun collectPropertyInfos(classDeclaration: KSClassDeclaration): List<PropertyInfo> {
        return classDeclaration.getDeclaredProperties()
            ...
            .mapNotNull { property ->
                val kind = determinePropertyKind(property) ?: return@mapNotNull null
                when (kind) {
                    PropertyKind.STRING -> {
                        val value = extractStringValue(property)
                        if (value == null) {
                            logger.warn("Skipping ${property.simpleName.asString()} – unable to resolve string literal")
                            null
                        } else {
                            PropertyInfo(property.simpleName.asString(), kind, listOf(value))
                        }
                    }
                    ...
                    PropertyKind.MUTABLE_SET -> {
                        ...
                        if (values == null) {
                            logger.warn("Skipping ${property.simpleName.asString()} – unable to resolve string collection literal")
                            null
                        } else {
                            PropertyInfo(property.simpleName.asString(), kind, values)
                        }
                    }
                }
            }
    ...
    private fun buildObfuscatedEntry(value: ObfuscatedValue): CodeBlock {
        val byteLiteral = if (value.bytes.isEmpty()) {
            "byteArrayOf()"
        } else {
            val joined = value.bytes.joinToString(", ") { it.toString() }
            "byteArrayOf($joined)"
        }

        return CodeBlock.of("(%L to %L)", byteLiteral, "${value.key}.toByte()")
    }
        ...
        return FunSpec.builder("deobfuscate")
            ...
            .addCode(
                ...
                val unshuffled = encoded.map { byte ->
                    ...
                    ((value shr 3) or (value shl 5)) and 0xFF
    ...
    private fun String.toAccessorSuffix(): String {
        ...
        return buildString(length) {
            ...
            for (char in this@toAccessorSuffix) {
                when {
